...
namespace MediaDevices
{
    ...
    public class MediaDeviceStatusService : MediaDeviceService
    {
        ...
        protected override void Update()
        {
            ...
            keyCol.Add(in WPD.SignalStrength);
            keyCol.Add(in WPD.TextMessages);
            keyCol.Add(in WPD.NewPictures);
            keyCol.Add(in WPD.MissedCalls);
            keyCol.Add(in WPD.VoiceMail);
            keyCol.Add(in WPD.NetworkName);
            keyCol.Add(in WPD.NetworkType);
            keyCol.Add(in WPD.Roaming);
            keyCol.Add(in WPD.BatteryLife);
            keyCol.Add(in WPD.ChargingState);
            keyCol.Add(in WPD.StorageCapacity);
            keyCol.Add(in WPD.StorageFreeSpace);
            keyCol.Add(in WPD.InternetConnected);
            IPortableDeviceValues values = GetProperties(keyCol);

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.SignalStrength, out value.Value);
                this.SignalStrength = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.TextMessages, out value.Value);
                this.TextMessages = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.NewPictures, out value.Value);
            ...
            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.MissedCalls, out value.Value);
                this.MissedCalls = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.VoiceMail, out value.Value);
                this.VoiceMail = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.NetworkName, out value.Value);
                this.NetworkName = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.NetworkType, out value.Value);
                this.NetworkType = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.Roaming, out value.Value);
                this.Roaming = (Roaming)(byte)value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.BatteryLife, out value.Value);
                this.BatteryLife = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.ChargingState, out value.Value);
                this.ChargingState = (ChargingState)(byte)value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.StorageCapacity, out value.Value);
                this.StorageCapacity = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.StorageFreeSpace, out value.Value);
                this.StorageFreeSpace = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                values.GetValue(in WPD.InternetConnected, out value.Value);
