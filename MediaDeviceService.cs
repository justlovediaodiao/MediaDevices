...
namespace MediaDevices
{
    ...
    public class MediaDeviceService : IDisposable
    {
        ...
        internal IPortableDeviceService service = ComActivationHelper.CreatePortableDeviceService();
        ...
        internal MediaDeviceService(MediaDevice device, string serviceId)
        {
            ...
            IPortableDeviceValues values = ComActivationHelper.CreatePortableDeviceValues();
...
#pragma warning disable IDE0090 // Use 'new(...)'
            ...
            {
                deviceValues.GetValue(in WPD.OBJECT_NAME, out value.Value);
                this.Name = value;
            }

            using (PropVariantFacade value = new PropVariantFacade())
            {
                deviceValues.GetValue(in WPD.FUNCTIONAL_OBJECT_CATEGORY, out value.Value);
            ...
            using (PropVariantFacade value = new PropVariantFacade())
            {
                deviceValues.GetValue(in WPD.SERVICE_VERSION, out value.Value);
...
#pragma warning restore IDE0060 // Remove unused parameter
        {
            ...
            IPortableDeviceValues results = ComActivationHelper.CreatePortableDeviceValues();
            methods.Invoke(in method, ref values, ref results);
        }

        internal void SendCommand(PropertyKey commandKey)
        {
            IPortableDeviceValues values = ComActivationHelper.CreatePortableDeviceValues();
            values.SetGuidValue(in WPD.PROPERTY_COMMON_COMMAND_CATEGORY, in commandKey.fmtid);
            values.SetUnsignedIntegerValue(in WPD.PROPERTY_COMMON_COMMAND_ID, commandKey.pid);
