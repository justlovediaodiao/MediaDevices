...
namespace MediaDevices
{
    ...
    public sealed class MediaDevice : IDisposable
    {
        ...
        static MediaDevice()
        {
            try
            {
                deviceManager = ComActivationHelper.CreatePortableDeviceManager();
                serviceManager = ComActivationHelper.QueryInterface<IPortableDeviceServiceManager>(
                    deviceManager, ComActivationHelper.IID_IPortableDeviceServiceManager);
        ...
        private MediaDevice(string deviceId)
        {
            ...
            {
                count = 256;
                IntPtr buffer = Marshal.AllocCoTaskMem((int)count * 2);
                try
                {
                    deviceManager.GetDeviceDescription(deviceId, buffer, ref count);
                    this.Description = Marshal.PtrToStringUni(buffer);
                }
                finally
                {
            ...
            try
            {
                ...
                try
                {
                    deviceManager.GetDeviceFriendlyName(deviceId, buffer, ref count);
                    this.friendlyName = Marshal.PtrToStringUni(buffer);
                }
                finally
                {
                    Marshal.FreeCoTaskMem(buffer);
            ...
            try
            {
                ...
                try
                {
                    deviceManager.GetDeviceManufacturer(deviceId, buffer, ref count);
                    this.Manufacturer = Marshal.PtrToStringUni(buffer);
                }
                finally
                {
                    Marshal.FreeCoTaskMem(buffer);
            ...
            this.device = ComActivationHelper.CreatePortableDevice();
        }
        ...
        public string FriendlyName
        {
            ...
            set
            {
                ...
                IPortableDeviceValues devInValues = ComActivationHelper.CreatePortableDeviceValues();
                devInValues.SetStringValue(in WPD.DEVICE_FRIENDLY_NAME, value);
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
        ...
        public void Connect(MediaDeviceAccess access = MediaDeviceAccess.Default, MediaDeviceShare share = MediaDeviceShare.Default, bool enableCache = false)
        {
            ...
            var clientInfo = ComActivationHelper.CreatePortableDeviceValues();
            clientInfo.SetStringValue(in WPD.CLIENT_NAME, appName);

            clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_MAJOR_VERSION, 1);
            clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_MINOR_VERSION, 0);
            clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_REVISION, 0);
            // Some device drivers need to impersonate the caller in order to function correctly. Since our application does not
            // need to restrict its identity, specify SECURITY_IMPERSONATION so that we work with all devices.
            clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_SECURITY_QUALITY_OF_SERVICE, (uint)Security.IMPERSONATION);


            if (access != MediaDeviceAccess.Default)
            {
                clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_DESIRED_ACCESS, (uint)access);
            }
            if (share != MediaDeviceShare.Default)
            {
                clientInfo.SetUnsignedIntegerValue(in WPD.CLIENT_SHARE_MODE, (uint)share);
            }
            if (enableCache == false)
            {
                // disable file list cache
                clientInfo.SetGuidValue(in WPD.CLIENT_EVENT_COOKIE, in WPD.CLSID_PORTABLE_DEVICES);
        ...
        public IEnumerable<string> FunctionalObjects(FunctionalCategory functionalCategory)
        {
            ...
            try
            {
                ...
                this.deviceCapabilities.GetFunctionalObjects(in guid, out IPortableDevicePropVariantCollection objects);
        ...
        public IEnumerable<ContentType> SupportedContentTypes(FunctionalCategory functionalCategory)
        {
            ...
            {
                Guid guid = functionalCategory.Guid();
                this.deviceCapabilities.GetSupportedContentTypes(in guid, out IPortableDevicePropVariantCollection types);
        ...
        internal void CallEvent(IPortableDeviceValues eventParameters)
        {
            ...
            eventParameters.GetGuidValue(in WPD.EVENT_PARAMETER_EVENT_ID, out Guid eventGuid);
        ...
        public MediaStorageInfo GetStorageInfo(string storageObjectId)
        {
            ...
            IPortableDeviceKeyCollection keys = ComActivationHelper.CreatePortableDeviceKeyCollection();
            keys.Add(in WPD.STORAGE_TYPE);
            keys.Add(in WPD.STORAGE_FILE_SYSTEM_TYPE);
            keys.Add(in WPD.STORAGE_CAPACITY);
            keys.Add(in WPD.STORAGE_FREE_SPACE_IN_BYTES);
            keys.Add(in WPD.STORAGE_FREE_SPACE_IN_OBJECTS);
            keys.Add(in WPD.STORAGE_DESCRIPTION);
            keys.Add(in WPD.STORAGE_SERIAL_NUMBER);
            keys.Add(in WPD.STORAGE_MAX_OBJECT_SIZE);
            keys.Add(in WPD.STORAGE_CAPACITY_IN_OBJECTS);
            keys.Add(in WPD.STORAGE_ACCESS_CAPABILITY);
        ...
        public IEnumerable<MediaDeviceService> GetServices(MediaDeviceServices service)
        {
            ...
            uint num = 0;
            serviceManager.GetDeviceServices(this.DeviceId, in serviceGuid, null, ref num);
            ...
            serviceManager.GetDeviceServices(this.DeviceId, in serviceGuid, services, ref num);
