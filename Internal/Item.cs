...
namespace MediaDevices.Internal
{
    ...
    internal class Item
    {
        ...
        static Item()
        {
            ...
            keyCollection = ComActivationHelper.CreatePortableDeviceKeyCollection();
            keyCollection.Add(in WPD.OBJECT_CONTENT_TYPE);
            keyCollection.Add(in WPD.OBJECT_NAME);
            keyCollection.Add(in WPD.OBJECT_ORIGINAL_FILE_NAME);

            keyCollection.Add(in WPD.OBJECT_HINT_LOCATION_DISPLAY_NAME);
            keyCollection.Add(in WPD.OBJECT_CONTAINER_FUNCTIONAL_OBJECT_ID);
            keyCollection.Add(in WPD.OBJECT_SIZE);
            keyCollection.Add(in WPD.OBJECT_DATE_CREATED);
            keyCollection.Add(in WPD.OBJECT_DATE_MODIFIED);
            keyCollection.Add(in WPD.OBJECT_DATE_AUTHORED);
            keyCollection.Add(in WPD.OBJECT_CAN_DELETE);
            keyCollection.Add(in WPD.OBJECT_ISSYSTEM);
            keyCollection.Add(in WPD.OBJECT_ISHIDDEN);
            keyCollection.Add(in WPD.OBJECT_IS_DRM_PROTECTED);
            keyCollection.Add(in WPD.OBJECT_PARENT_ID);
            keyCollection.Add(in WPD.OBJECT_PERSISTENT_UNIQUE_ID);
        }
        ...
        public static Item GetFromPersistentUniqueId(MediaDevice device, string persistentUniqueId)
        {
            ...
            var collection = ComActivationHelper.CreatePortableDevicePropVariantCollection();

            using (var propVariantPUID = PropVariantFacade.StringToPropVariant(persistentUniqueId))
            {
                collection.Add(in propVariantPUID.Value);
            }
        ...
        internal Item CreateSubdirectory(string path)
        {
            ...
            foreach (var folder in folders)
            {
                ...
                if (child == null)
                {
                    ...
                    IPortableDeviceValues deviceValues = ComActivationHelper.CreatePortableDeviceValues();
                    deviceValues.SetStringValue(in WPD.OBJECT_PARENT_ID, parent.Id);
                    deviceValues.SetStringValue(in WPD.OBJECT_NAME, folder);
                    deviceValues.SetStringValue(in WPD.OBJECT_ORIGINAL_FILE_NAME, folder);
                    deviceValues.SetGuidValue(in WPD.OBJECT_CONTENT_TYPE, in WPD.CONTENT_TYPE_FOLDER);
        ...
        {
            var objectIdCollection = ComActivationHelper.CreatePortableDevicePropVariantCollection();

            var propVariantValue = PropVariantFacade.StringToPropVariant(this.Id);
            objectIdCollection.Add(in propVariantValue.Value);

            IPortableDevicePropVariantCollection results = ComActivationHelper.CreatePortableDevicePropVariantCollection();
        ...
        internal Stream OpenRead()
        {
            ...
            resources.GetStream(this.Id, in WPD.RESOURCE_DEFAULT, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal Stream OpenReadThumbnail()
        {
            ...
            resources.GetStream(this.Id, in WPD.RESOURCE_THUMBNAIL, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal Stream OpenReadIcon()
        {
            ...
            resources.GetStream(this.Id, in WPD.RESOURCE_ICON, 0, ref optimalTransferSize, out wpdStream);
        ...
        internal void UploadFile(string fileName, Stream stream)
        {
            ...
            IPortableDeviceValues portableDeviceValues = ComActivationHelper.CreatePortableDeviceValues();

            portableDeviceValues.SetStringValue(in WPD.OBJECT_PARENT_ID, this.Id);
            portableDeviceValues.SetUnsignedLargeIntegerValue(in WPD.OBJECT_SIZE, (ulong)stream.Length);
            portableDeviceValues.SetStringValue(in WPD.OBJECT_ORIGINAL_FILE_NAME, fileName);
            portableDeviceValues.SetStringValue(in WPD.OBJECT_NAME, fileName);
            // test
            using (PropVariantFacade now = PropVariantFacade.DateTimeToPropVariant(DateTime.Now))
            {
                portableDeviceValues.SetValue(in WPD.OBJECT_DATE_CREATED, in now.Value);
                portableDeviceValues.SetValue(in WPD.OBJECT_DATE_MODIFIED, in now.Value);
        ...
        internal bool Rename(string newName)
        {
            ...
            portableDeviceValues.SetStringValue(in WPD.OBJECT_ORIGINAL_FILE_NAME, newName);
        ...
        internal void SetDateCreated(DateTime value)
        {
            ...
            {
                portableDeviceValues.SetValue(in WPD.OBJECT_DATE_CREATED, in val.Value);
                this.device.deviceProperties.SetValues(this.Id, portableDeviceValues, out result);
                ComTrace.WriteObject(result);
            }
        ...
        internal void SetDateModified(DateTime value)
        {
            ...
            using (PropVariantFacade val = PropVariantFacade.DateTimeToPropVariant(value))
            {
                portableDeviceValues.SetValue(in WPD.OBJECT_DATE_MODIFIED, in val.Value);
                this.device.deviceProperties.SetValues(this.Id, portableDeviceValues, out result);
                ComTrace.WriteObject(result);
            }
        ...
        internal void SetDateAuthored(DateTime value)
        {
            ...
            using (PropVariantFacade val = PropVariantFacade.DateTimeToPropVariant(value))
            {
                portableDeviceValues.SetValue(in WPD.OBJECT_DATE_AUTHORED, in val.Value);
                this.device.deviceProperties.SetValues(this.Id, portableDeviceValues, out result);
                ComTrace.WriteObject(result);
            }
